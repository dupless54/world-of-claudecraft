...
// Rebuild the manifest from whatever is on disk. Detects numbered variants
// (<key>_1.mp3, <key>_2.mp3 …) and groups them under their base key so the
// engine can pick among them at runtime.
function buildManifest() {
  ...
  for (const entry of SFX) {
    ...
    if (urls.length === 0 && existsSync(path.join(sfxDir, `${entry.key}.mp3`))) {
      urls.push(`/audio/sfx/${entry.key}.mp3`);
    }
...
if (manifestOnly) {
  ...
  console.log(`Manifest rebuilt: ${path.relative(root, manifestPath)} (${n} keys).`);
  process.exit(0);
}
